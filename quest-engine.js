...
function playHevClick() {
    ...
    const gain = audio// --- 1. ЗВУК И ГЛОБАЛЫ ---
const audioContext = new (window.AudioContext || window.webkitAudioContext)();
function playHevClick() {// --- 1. ЗВУК И ПЕРЕМЕННЫЕ ---
const audioContext = new (window.AudioContext || window.webkitAudioContext)();
function playHevClick() {
    if (audioContext.state === 'suspended') audioContext.resume();
    const osc = audioContext.createOscillator();
...
const questTargets = [8, 26, 47, 79]; // O, Fe, Ag, Au
let discovered = 0, timerInterval, isHayBurned = false;
let snake, food, direction, gameLoop, box = 20, snakeTimeLeft = 120;
let kills = 0, galaxyInterval, currentTargetId = null, blinkInterval;
...
function handleQuest(id, div) {
    if (id === currentTargetId && !div.classList.contains('stabilized')) {
        ...
        if (blinkInterval) clearTimeout(blinkInterval);
        if (discovered < 4) {
            setTimeout(startElementHunt, 500);
        } else {
            setTimeout(() => {
                document.querySelector('.table-viewport').innerHTML = `
...
function checkLogic(item) {
    ...
    if (item === 'matches') { isHayBurned = true; hint.innerHTML = "СЕНО СГОРЕЛО. ОСТАЛСЯ ПЕПЕЛ."; }
        ...
        if (isHayBurned) { hint.innerHTML = "КЛЮЧ НАЙДЕН! ПЕРЕХОД..."; setTimeout(() => setState('BIOLOGY'), 1500); }
        else hint.innerHTML = "СЛИШКОМ МНОГО СЕНА!";
    }
}
...
function startSnakeGame() {
    ...
    gameLoop = setInterval(() => {
        snakeTimeLeft -= 0.2;
        if (snakeTimeLeft <= 0) { clearInterval(gameLoop); setState('GALAXY'); return; }
...
// --- 7. ФИНАЛ ---
function showDetails(el) { document.getElementById('content').innerHTML = `<h2>${el.name}</h2><p>Масса: ${el.m}</p>`; document.getElementById('details').classList.add('open'); }
function closeDetails() { document.getElementById('details').classList.remove('open'); }
function startTimer() {
    let tl = 900;
    if (timerInterval) clearInterval(timerInterval);
...
window.onload = () => setState('MENU');Context.createGain();
    osc.type = 'sine'; osc.frequency.setValueAtTime(800, audioContext.currentTime);
    gain.gain.exponentialRampToValueAtTime(0.01, audioContext.currentTime + 0.1);
    osc.connect(gain); gain.connect(audioContext.destination);
    osc.start(); osc.stop(audioContext.currentTime + 0.1);
